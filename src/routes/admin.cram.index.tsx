    ...
    >
      <div className="grid gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Active Parameters", value: "24", icon: Zap, color: "text-primary", bg: "bg-primary/10" },
            { label: "Pending Approvals", value: "03", icon: AlertTriangle, color: "text-warning", bg: "bg-warning/10" },
            { label: "Active Workflows", value: "05", icon: ShieldCheck, color: "text-success", bg: "bg-success/10" },
            { label: "System Version", value: "v2.1", icon: History, color: "text-muted-foreground", bg: "bg-muted" },
          ].map((stat, i) => (
          ...
          <aside className="space-y-6">
            <Panel title="Pending Actions">
              <div className="space-y-4 pt-2">
                {[
                  { title: "Risk Weight Update", desc: "v2.2 requires approval", type: "Checker", time: "2h ago" },
                  { title: "New Industry Parameter", desc: "Added by Compliance Admin", type: "Approver", time: "5h ago" },
                  { title: "Decision Matrix Change", desc: "v1.5 draft needs review", type: "Maker", time: "1d ago" },
                ].map((action, i) => (
                  <div key={i} className="p-3 bg-muted/20 border border-border/50 rounded-xl hover:border-primary/30 transition-colors cursor-pointer">
                    <div className="flex justify-between items-start mb-1">
                      <h4 className="text-xs font-bold">{action.title}</h4>
                      <Badge variant="secondary" className="text-[8px]">{action.type}</Badge>
                    </div>
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" className="h-6 text-[9px] font-bold uppercase tracking-widest text-primary">Review</Button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Panel title="Configuration Overview">
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs p-2 rounded hover:bg-muted/30 cursor-pointer group">
            ...
            <div className="bg-gradient-to-br from-primary/10 to-transparent p-5 rounded-2xl border border-primary/20">
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                All changes are currently following the 2-level approval policy. Audit logs are being streamed to the Central Security Vault.
              </p>
            </div>
          </aside>
        </div>
