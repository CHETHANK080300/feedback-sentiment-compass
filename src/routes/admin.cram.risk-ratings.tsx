...
function RiskRatings() {
    ...
    const validateRanges = () => {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (current.minScore > current.maxScore) {
          newErrors.push(
            `${current.rating}: Min score (${current.minScore}) cannot be greater than max score (${current.maxScore}).`,
          );
        }
        ...
        if (i > 0) {
          ...
          if (current.minScore <= prev.maxScore) {
            newErrors.push(
              `Overlap detected between ${prev.rating} and ${current.rating}.`,
            );
          } else if (current.minScore > prev.maxScore + 1) {
            newErrors.push(
              `Gap detected between ${prev.rating} and ${current.rating}.`,
            );
          }
        }
      }
  ...
  const updateRange = (
    ...
    setRatings(
      ratings.map((r) => (r.id === id ? { ...r, [field]: numValue } : r)),
    );
  };
  ...
  const getRiskLevelBadge = (level: string) => {
    switch (level) {
      ...
      case "High":
        return <Badge className="bg-critical/80 text-white">High</Badge>;
      case "Very High":
        return <Badge className="bg-critical text-white">Very High</Badge>;
      default:
        ...
        {errors.length > 0 ? (
          ...
          <Alert className="bg-success/10 border-success/20 text-success">
            ...
            <AlertDescription className="text-xs">
              Rating ranges are continuous, non-overlapping, and cover the full
              0-100 spectrum.
            </AlertDescription>
          </Alert>
        )}
                ...
                >
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) =>
                        updateRange(rating.id, "minScore", e.target.value)
                      }
                  ...
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) =>
                        updateRange(rating.id, "maxScore", e.target.value)
                      }
                      className="w-24 font-mono font-bold"
                    />
                  </TableCell>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-muted/30 p-5 rounded-xl border border-border border-dashed">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                {errors.some((e) => e.includes("0") || e.includes("100")) ? (
                  <AlertTriangle className="h-3 w-3 text-critical" />
                ) : (
                  <CheckCircle2 className="h-3 w-3 text-success" />
                )}
          ...
          <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Score ranges are inclusive. The system ensures that a score of
              30.5 would be handled based on the integer range mapping.
              Continuous validation prevents misconfiguration that could lead to
              "Unrated" risk assessments.
            </p>
          </div>
        </div>
      </div>
