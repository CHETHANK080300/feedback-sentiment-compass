...
import { CustomerRiskAssessment, RiskLevel, DecisionType } from "@/lib/cram-types";
import {
  ResponsiveContainer,
  PieChart,
  Pie,
...
const getDecisionBadge = (decision: DecisionType) => {
  switch (decision) {
    case 'Approve': return <Badge className="bg-success text-white">Approve</Badge>;
    case 'Review': return <Badge className="bg-warning text-white">Review</Badge>;
    case 'Reject': return <Badge className="bg-critical text-white">Reject</Badge>;
    case 'Escalate': return <Badge className="bg-primary text-white">Escalate</Badge>;
  }
};

function CustomerAssessments() {
  const [selectedAssessment, setSelectedAssessment] = useState<CustomerRiskAssessment | null>(null);
          ...
          <Table>
            ...
            <TableBody>
              {mockAssessments.map((as) => (
                <TableRow key={as.id} className="hover:bg-muted/10 transition-colors group">
                  <TableCell>
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{as.customerId} • {as.customerType}</div>
                    </div>
                  </TableCell>
                  <TableCell>{getRiskLevelBadge(as.rating)}</TableCell>
                  <TableCell>
                    <div className="font-mono font-bold text-lg">{as.finalScore}</div>
                  </TableCell>
...
function AssessmentDetail({ assessment, onBack }: { assessment: CustomerRiskAssessment, onBack: () => void }) {
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-card rounded-2xl border border-border p-6 shadow-sm relative overflow-hidden">
            <div className={`absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 rounded-full opacity-10 ${
              assessment.rating === 'Very High' ? 'bg-critical' :
              assessment.rating === 'High' ? 'bg-critical' :
              assessment.rating === 'Medium' ? 'bg-warning' : 'bg-success'
            }`} />

            <div className="relative">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="text-2xl font-bold">{assessment.finalScore}</h3>
                  <div className="text-xs text-muted-foreground font-bold uppercase tracking-widest mt-1">Final Risk Score</div>
                </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[10px] uppercase font-bold text-muted-foreground">Assessment Date</div>
                    <div className="text-sm font-medium">{new Date(assessment.assessmentDate).toLocaleString()}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-muted rounded-lg"><Activity className="h-4 w-4 text-muted-foreground" /></div>
                  <div>
              ...
              <Button className="w-full mt-6 gap-2" variant="outline" onClick={() => toast.success("PDF Report Generated")}>
          ...
          <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-6 shadow-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-y-6 gap-x-12">
              <div className="space-y-1">
                <div className="text-[10px] uppercase font-bold text-muted-foreground">Customer Name</div>
                <div className="font-medium text-foreground">{assessment.customerName}</div>
              </div>
              <div className="space-y-1">
                <div className="text-[10px] uppercase font-bold text-muted-foreground">Customer ID / CIF</div>
              ...
              <div className="space-y-1">
                ...
                <div className="text-sm font-medium">{assessment.industry} · {assessment.segment}</div>
              </div>
              <div className="space-y-1">
                <div className="text-[10px] uppercase font-bold text-muted-foreground flex items-center gap-1.5">
                  <Globe className="h-3 w-3" /> Geography
                </div>
                <div className="text-sm font-medium">{assessment.geography}</div>
              </div>
              <div className="col-span-2 space-y-1">
                <div className="text-[10px] uppercase font-bold text-muted-foreground">Customer Type</div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-xs font-bold">Risk Parameter</TableHead>
                    <TableHead className="text-xs font-bold text-center">Base Score</TableHead>
                    <TableHead className="text-xs font-bold text-center">Weight</TableHead>
                    <TableHead className="text-xs font-bold text-right">Weighted Score</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {assessment.breakdown.map((item, i) => (
                    <TableRow key={i}>
                      <TableCell className="text-sm">{item.parameterName}</TableCell>
                      <TableCell className="text-center font-mono">{item.score}</TableCell>
                      <TableCell className="text-center font-mono text-muted-foreground">{item.weight}%</TableCell>
                      <TableCell className="text-right font-mono font-bold text-primary">{item.weightedScore.toFixed(1)}</TableCell>
                    </TableRow>
                  ))}
                  <TableRow className="bg-muted/20">
                    <TableCell className="font-bold" colSpan={3}>Final Weighted Score</TableCell>
                    <TableCell className="text-right font-bold text-lg">{assessment.finalScore}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-card rounded-2xl border border-border p-6 shadow-sm">
              <h3 className="font-bold text-sm mb-4 flex items-center gap-2">
                <PieChartIcon className="h-4 w-4 text-primary" /> Risk Contribution
              </h3>
              <div className="h-48">
