          ...
          <div className="flex gap-4 items-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Governance active for 5 modules
              </p>
            </div>
          </div>
            ...
            {mockWorkflows.map((wf) => (
              ...
              >
                    ...
                    {wf.steps.map((step, idx) => (
                      ...
                      >
                        <div className="flex flex-col items-center gap-2">
                          <div
                            className={`h-10 w-10 rounded-full flex items-center justify-center border-2 ${idx === 0 ? "bg-warning/10 border-warning text-warning" : "bg-primary/10 border-primary text-primary"}`}
                          >
                            {idx === 0 ? (
                ...
                <div className="px-6 py-4 bg-muted/10 border-t border-border flex flex-wrap gap-x-8 gap-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`h-2 w-2 rounded-full ${wf.status === "Active" ? "bg-success" : "bg-muted"}`}
                    />
          ...
          <aside className="space-y-6">
            <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
              <h3 className="font-bold text-sm mb-4 flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 text-warning" /> Governance
                Rules
              </h3>
              <div className="space-y-4">
                <div className="p-3 bg-muted/20 rounded-lg border border-border/50">
                  ...
                  <p className="text-xs text-muted-foreground">
                    The person who initiates a change cannot be the one who
                    approves it.
                  </p>
                </div>
                <div className="p-3 bg-muted/20 rounded-lg border border-border/50">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Pending requests older than specified time are escalated to
                    supervisors.
                  </p>
                </div>
                <div className="p-3 bg-muted/20 rounded-lg border border-border/50">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Every step in the workflow is logged with user, IP, and
                    timestamp.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-primary/10 p-5 rounded-2xl border border-primary/20">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                All CRAM configuration changes must follow at least a 2-level
                approval workflow for Production environments.
              </p>
